  ...
  <div class="form-area">
	<div id="tab-control-meta" class="tab_control">
		...
		<div id="meta-pages" class="pages">
			...
			<!-- </div>
			<div id="div-metadata" title="<?php echo __('Options'); ?>">
			  <table cellpadding="0" cellspacing="0" border="0"> -->
				<tr>
				  <td class="label"><label for="page_breadcrumb" title="Page text used for descriptive links (typically link text in menus)"><?php echo __('Name'); ?></label></td>
				  <td class="field"><input class="textbox" title="Page text used for descriptive links (typically link text in menus)" id="page_breadcrumb" maxlength="160" name="page[breadcrumb]" size="160" type="text" value="<?php echo htmlentities($page->breadcrumb, ENT_COMPAT, 'UTF-8'); ?>" /></td>
				</tr>
				<?php if ($page->parent_id != 0) : ?>
				<tr class="page-uri">
				  <td class="label"><label for="page_slug" title="Page filename as displayed in browser address bar"><?php echo __('URI'); ?></label></td>
				...
				<?php if(Plugin::isEnabled('tagger') == true){ ?>
				<tr>
				  <td class="label optional"><label for="page_tags" title="Keywords unique and relative to page"><?php if(Plugin::isEnabled('form_core') == true && ((AuthUser::hasPermission('administrator') && AuthUser::hasPermission('developer')) || (AuthUser::hasPermission('administrator') && AuthUser::hasPermission('editor')))){ ?><a href="/<?php echo ADMIN_DIR; ?>/plugin/tagger" title="Keywords/tags"><?php } ?><?php echo __('Tags'); ?><?php if(Plugin::isEnabled('form_core') == true && ((AuthUser::hasPermission('administrator') && AuthUser::hasPermission('developer')) || (AuthUser::hasPermission('administrator') && AuthUser::hasPermission('editor')))){ ?></a><?php } ?></label></td>
				  <td class="field"><input class="textbox" id="page_tags" maxlength="255" name="page_tag[tags]" size="255" title="Keywords unique and relative to page" type="text" value="<?php echo join(', ', $tags); ?>" /></td>
				</tr>
				<?php } ?>
			  <!--</table>
			</div>
			<div id="div-settings" title="<?php echo __('Settings'); ?>">
			  ...
			  -->
				...
				<tr>
				  <td class="label"><label for="page_behavior_id"><?php echo __('Type'); ?></label></td>
				  <td class="field">
			  ...
			  <?php if (isset($page->created_on)): ?>
				<tr>
				  <td class="label"><label for="page_created_on"><?php echo __('Created'); ?></label></td>
				  <td class="field">
				...
				<?php if (isset($page->published_on)): ?>
					</td></tr><tr><td class="label"><label for="page_published_on"><?php echo __('Published'); ?></label></td><td class="field">
			  ...
			  <?php if (!AuthUser::hasPermission('client')){ ?>
				<tr>
				  <td class="label"><label for="page_needs_login" title="<?php echo __('When enabled, users have to login before they can view the page.'); ?>"><?php echo __('Login:'); ?></label></td>
				  <td class="field">
					...
					<input id="page_is_protected" name="page[is_protected]" class="checkbox" type="checkbox" value="1"<?php if ($page->is_protected) echo ' checked="checked"'; ?>/><label for="page_is_protected" title="<?php echo __('When enabled, only users who are an administrator can edit the page.'); ?>" class="protected"> <?php echo __('Protected'); ?> </label>
				  </td>
				</tr>
			  <?php } else { ?>

				<input id="page_needs_login" name="page[needs_login]" type="hidden" value="<?php echo $page->needs_login; ?>" />
	...
	<?php
		// Check if page has form parts enabled
		$pagepartform = false;
		if(defined('PAGE_PART_FORM') && PAGE_PART_FORM == 'enabled') $pagepartform = true;
	?>
...
<?php if($pagepartform == false){ // Display status here, if not custom part part form ?>
<?php if ( ! isset($page->id) || $page->id != 1): ?>
	  <div class="boxpart">
	  <p id="pagestatus"><label for="page_status_id"><?php echo __('Status'); ?></label>
...
<?php if ( $page->id == 1): ?>
	  <div class="boxpart">
	  <p id="pagestatus"><label for="page_status_id"><?php echo __('Status'); ?></label>
	  	 <select id="page_status_id" name="page[status_id]" disabled="disabled">
		   <option value="<?php echo Page::STATUS_PUBLISHED; ?>"<?php echo $page->status_id == Page::STATUS_PUBLISHED ? ' selected="selected"': ''; ?>><?php echo __('Published'); ?></option>
		 </select>
	  </p>
	  </div>
...
<?php if($pagepartform == true){ // Display status here, if custom part part form ?>
<div id="pagepartformpages" class="pagepartformpages">
<br />
<?php if ( ! isset($page->id) || $page->id != 1): ?>
	  <div class="boxpart">
	  <p id="pagestatus"><label for="page_status_id"><?php echo __('Status'); ?></label>
...
<?php if ( $page->id == 1): ?>
	  ...
	  </div><?php endif; // Display publish status element on home page - GUI consistence enhancement ?>
</div>
<?php } // ?>
...
<?php endif; ?>
  ...
  <p class="buttons">
	...
	<?php if ($action == 'edit') { ?><a class="button" id="site-view-page" onclick="target='_blank'" onkeypress="target='_blank'" href="<?php echo URL_PUBLIC; echo (USE_MOD_REWRITE == false) ? '?' : ''; echo $page->getUri(); if(stristr($page->slug, '.') === FALSE){ $SUFFIX = URL_SUFFIX; } else { $SUFFIX = ''; }echo ($page->getUri() != '') ? $SUFFIX : ''; ?>" title="<?php echo __('Preview this page'); ?>"><?php echo __('Preview'); ?></a><?php } ?></p>
