...
class PageController extends Controller {
    ...
    private function _store($action, $id=false) {
        ...
        if (isset($_POST['csrf_token'])) {
            ...
            $feedback = 'Token: '.$csrf_token." URL: ".BASE_URL.'page/'.$action.$csrf_id;
            if (!SecureToken::validateToken($csrf_token, BASE_URL.'page/'.$action.$csrf_id)) {
                $errors[] = __('Invalid CSRF token found! Can not store! '.$feedback);
            } else {
				//$errors[] = __('Token Success! '.$feedback);
			}
        }
        else {
        ...
        if(isset($data['slug'])){ $data['slug'] = trim($data['slug']); } else { $data['slug'] = ''; }
        if (empty($data['slug']) && $id != '1') {
            ...
            if (!Validate::slug($data['slug']) && (!empty($data['slug']) && $id == '1')) {
                $errors[] = __('Illegal value for :fieldname slug!', array(':fieldname' => 'slug'));
            }
        }
        ...
        foreach ($fields as $field) {
            if (!Validate::digit($data[$field])) {
                $errors[] = __('Illegal value for :fieldname id field: '.$data[$field], array(':fieldname' => $field));
            } else {
				//$errors[] = __('Success for :fieldname id field: '.$data[$field], array(':fieldname' => $field));
			}
        }
        ...
        foreach ($fields as $field) {
            if (isset($data[$field])) {
                ...
                if (!empty($data[$field]) && !(bool) preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/D', (string) $data[$field])) {
                    $errors[] = __('Illegal value for :fieldname date field!', array(':fieldname' => $field));
                }
            }
        }
        ...
        foreach ($fields as $field) {
            if (isset($data[$field])) {
                ...
                if (!empty($data[$field]) && !(bool) preg_match('/^[0-9]{2}:[0-9]{2}:[0-9]{2}$/D', (string) $data[$field])) {
                    $errors[] = __('Illegal value for :fieldname time field!', array(':fieldname' => $field));
                }
            }
        }
        ...
        foreach ($fields as $field) {
            use_helper('Kses');
            if(isset($data[$field])){ $data[$field] = kses(trim($data[$field]), array()); } else { $data[$field] = ''; }
            /*
        ...
        if (!empty($data['behaviour_id']) && !Validate::slug($data['behaviour_id'])) {
            $errors[] = __('Illegal value for :fieldname behaviour field!', array(':fieldname' => 'behaviour_id'));
        }
        ...
        if (false !== $errors) {
            if(isset($_POST['page_tag'])){ $tags = $_POST['page_tag']; } else { $_POST['page_tag'] = ''; }
        ...
        if ($page->save()) {
            ...
            // save tags
            if(isset($_POST['page_tag']['tags'])) $page->saveTags($_POST['page_tag']['tags']);
