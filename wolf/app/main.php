...
function main() {
	...
	// If we're using mod_rewrite, we should have a THISPAGE entry.
	if (USE_MOD_REWRITE && array_key_exists('THISPAGE', $_GET)) {
		$uri = $_GET['THISPAGE'];
		unset($_GET['THISPAGE']);
	}
	else if (USE_MOD_REWRITE)   // We're using mod_rewrite but don't have a THISPAGE entry, assume site root.
			$uri = '';
	...
	if (is_object($page)) {
		...
		Observer::notify('page_found', $page); $page->includeSnippet('registerfunctions'); // Include core snippet
		$page->_executeLayout();
	}
	else {
		page_not_found();
