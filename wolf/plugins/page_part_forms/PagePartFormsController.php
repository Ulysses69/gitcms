...
class PagePartFormsController extends PluginController {
	...
	public static function Init() {
	  ...
	  if(!AuthUser::hasPermission('client')){
	  	if(Plugin::isEnabled('dashboard') == true) {
			// Register controller
			Plugin::addController(self::PLUGIN_ID, __('Page Profiles'), 'administrator', false);
		} else {
			Plugin::addController(self::PLUGIN_ID, __('Page Profiles'), 'administrator', true);
		}
	  }
  ...
  public static function callback_view_page($page) {
	...
	/* Get menu parent */
	$parentpage = explode('/', $_SERVER["REQUEST_URI"]);
	$parentid = $parentpage[sizeof($parentpage)-1];

	// Is this a newly generated page? Do nothing.
	if (!isset($page->id) && !isset($parentid)) {
	  return;
	}

	// Because the metadata is not visible, we can't use $page->metadata[self::PLUGIN_ID]
	$formid = PageMetadata::FindOneByPageAndKeyword($page->id, self::PLUGIN_ID);
	$formparentid = PageMetadata::FindOneByPageAndKeyword($page->parent_id, self::PLUGIN_ID.'_children');
	if($formparentid){ $form = $formparentid; } else { $form = $formid; }
	if($form){
