...
class BackupRestoreController extends PluginController {
	...
	function backup() {
		...
		// Define settings
		if(!isset($settings['pwd'])) $settings['pwd'] = '';
		if(!isset($settings['backupfiles'])) $settings['backupfiles'] = '';
		...
		if(AuthUser::hasPermission('client')){
			/* Restrict client users to following tables */
			$tablenames = array('page','page_metadata','page_part','page_part_forms','page_related_pages','plugin_settings','user','user_permission');
		}
		...
		$xmltext = '<?xml version="1.0" encoding="UTF-8"?><cms></cms>';
		...
		// Create the XML file
		$file = $xmlobj->asXML(); $filedate = strtolower(date("j-F-Y"));
		//$filename = 'wolfcms-backup-'.date($settings['stamp']).'.'.$settings['extension'];
		$filename = str_replace(' ','-',strtolower(Setting::get('admin_title'))).'-cms-'.$filedate;
