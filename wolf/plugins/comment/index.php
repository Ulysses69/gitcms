...
Plugin::setInfos(array(
	...
	'version'	 => '1.2.1',
	'license'	 => 'GPL',
	'author'	  => 'Philippe Archambault - Steven Henderson Modifications',
...
{
	echo '<p id="pagecomments"><label for="page_comment_status">'.__('Comments').'</label><select id="page_comment_status" name="page[comment_status]">';
...
function comment_display_moderatable_count(&$plugin_name, &$plugin)
{
	if ($plugin_name == 'comment')
	{
		...
		if ($numlabel && $numlabel == '1')
		{
			...
			if($pendingcount != '0'){
			if($pendingcount == '1'){ $pendings = ''; } else { $pendings = 's'; }
			//$plugin->label = $plugin->label.' <span id="comment-badge">'.comments_count_moderatable().'<span>/</span>'.comments_count_total().'</span>';
			$plugin->label = $plugin->label.' <span id="comment-badge" title="'.$pendingcount.' comment'.$pendings.' pending approval">'.$pendingcount.'</span>';
			}
		}
	}
}
...
<?php
	...
	if ($captcha && $captcha == '1') {
		if($data && ($data['secure'] != $_SESSION['security_number'] && !empty($data['secure'])) ) {
			echo '<p class="warning">'.__('Incorrect answer. Please try again: ').'</p>';
		}
		else {
			echo '<p>'.__('Solve the following security puzzle: ').'</p>';
		}
		if(function_exists('clienttelephone')){ $alt = 'Please call '.clienttelephone($page).' if you have any problems with our security question.';}else{ $alt = 'Pleas call us if you have any problems with our security question.';}
		echo '<img id="comment-captcha" src="'.URL_PUBLIC.'wolf/plugins/comment/image.php" alt="'.__($alt).'" />';
...
<p class="note">In submitting your information you agree to our <a href="/privacy.html">privacy policy</a>.</p>
<input type="submit" id="comment_form_submit" name="commit-comment" value="Post comment" class="submit" />
</div>
	...
	if (isset($_POST['commit-comment'])) {
		if (($captcha && $captcha != '1') || $data['secure'] == $_SESSION['security_number']) {
			if($approve && $approve == '1') {
				//echo '<p class="success" id="comment-thanks">'.__('Thank you for your comment. It has been added.').'</p>';
				echo '<!-- Comment success -->';
			}
			else {
				//echo '<p class="success" id="comment-thanks">'.__('Thank you for your comment. It is waiting for approval.').'</p>';
				echo '<!-- Comment success -->';
			}
		}
	}
