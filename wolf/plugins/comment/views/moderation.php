...
function commentPageTitle($commentTitle){
	...
	$commentTitle = str_replace('[client]',$clientname,$commentTitle);
	$commentTitle = str_replace('[clientname]',$clientname,$commentTitle);
	$commentTitle = str_replace('[address]',$clientaddress,$commentTitle);
	$commentTitle = str_replace('[telephone]',$clientphone,$commentTitle);
	$commentTitle = str_replace('[email]',$clientemail,$commentTitle);
	$commentTitle = str_replace('[location]',$clientlocation,$commentTitle);
	return ereg_replace("/\n\r|\r\n|\n|\r/", '', $commentTitle);
}
...
<?php while ($comment = $stmt->fetchObject()): ?>
	<li class="<?php echo odd_even(); ?> moderate">
		...
		if ($comment->author_email != '') {
			echo '<a href="mailto:'.$comment->author_email.'" title="'.$comment->author_name.'">'.$comment->author_name.'</a>';
		} else {
	...
	if(Plugin::isEnabled('clientdetails') == true){
		$commenttitle = commentPageTitle($comment->title);
	} else {
		$commenttitle = $comment->title;
	}
	?>
 <?php echo __('commented on'); ?> <strong><?php echo $commenttitle; ?></strong>
