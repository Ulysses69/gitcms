...
for($i=0;$i<$loopnum;$i++)
	...
	if(stristr($options[$i][1],'.svn') === FALSE && stristr($options[$i][1],'prop-base') === FALSE && stristr($options[$i][1],'props') === FALSE && stristr($options[$i][1],'text-base') === FALSE && stristr($options[$i][1],'tmp') === FALSE) echo '<option value="'.$options[$i][0].'"'.$selected.'>'.$options[$i][1].'</option>';
	}
?></select><?php
}
...
function mb_sql_regcase($item,$encoding='auto'){
	$ret='';$max=mb_strlen($item,$encoding);
	for ($i = 0; $i < $max; $i++) {
