/*globals alert,document,tinymce,tinyMCEMediaList,tinyMCEPopup,TinyMCE_EditableSelects,selectByValue,getBrowserHTML,getColorPickerHTML,isVisible,updateColor,AutoValidator*/
tinyMCEPopup.requireLangPack();
var oldWidth, oldHeight, ed, url = tinyMCEPopup.getParam("media_external_list_url");
function jsEncode(s) {
...
function setBool(pl, p, n) {
	if (typeof pl === "undefined" || typeof n !== "string" || typeof pl[n] === "undefined") {
		return;
	}
...
function getType(v) {
	...
	fo = ed.getParam("media_types", "video=mp4,ogv;flash=swf;flv=flv;shockwave=dcr;qt=mov,qt,mpg,mp3,mp4,mpeg;shockwave=dcr;wmp=avi,wmv,wm,asf,asx,wmx,wvx;rmp=rm,ra,ram").split(';');
	// YouTube
	if (v.match(/watch\?v=(.+)(.*)/)) {
		f.width.value = '425';
		f.height.value = '350';
...
function changedType(t) {
	...
	d.getElementById('video_options').style.display = 'none';
	if (t) {
...
function serializeParameters() {
	var d = document,
		f = d.forms[0],
		s = '',
		t = f.media_type.options[f.media_type.selectedIndex].value;
	switch (t) {
	case "video":
		s += getStr('video', 'mimetype');
		s += getStr('video', 'poster');
		s += getStr('video', 'title');
		s += getStr('video', 'media');
		s += getBool('video', 'showcontrols');
		s += getBool('video', 'autoplay');
		s += getBool('video', 'loop');
		s += getBool('video', 'preload');
		s += getBool('video', 'buffer');
		break;
	case "flash":
...
function generatePreview(c) {
	...
	p.innerHTML = '<!-- x --->';
	nw = parseInt(f.width.value, 10);
	nh = parseInt(f.height.value, 10);
	...
	pl = tinyMCEPopup.editor.plugins.mediahtml5._parse(pl);
	if (!pl.src) {
		p.innerHTML = '';
		return;
	}
	...
	if (f.media_type.options[f.media_type.selectedIndex].value !== "video") {
		...
		// video tag
		h += '<video';
		h += typeof pl.id !== "undefined" ? ' id="' + pl.id + '"' : '';
		h += typeof pl.poster !== "undefined" ? ' poster="' + pl.poster + '"' : '';
		h += typeof pl.title !== "undefined" ? ' title="' + pl.title + '"' : '';
		h += typeof pl.width !== "undefined" ? ' width="' + pl.width + '"' : '';
		h += typeof pl.height !== "undefined" ? ' height="' + pl.height + '"' : '';
		h += typeof pl.showcontrols === "boolean" && pl.showcontrols ? ' controls="controls"' : '';
		h += typeof pl.autoplay === "boolean" && pl.autoplay ? ' autoplay="autoplay"' : '';
		h += typeof pl.preload === "boolean" && pl.preload ? ' preload="auto"' : '';
		h += typeof pl.loop === "boolean" && pl.loop ? ' loop="true"' : '';
		h += typeof pl.autobuffer === "boolean" && pl.autobuffer ? ' autobuffer="autobuffer"' : '';
		h += '>';
		h += '<source src="' + pl.src;
		h += typeof pl.mimetype !== "undefined" ? '" type="' + pl.mimetype + '"' : '';
		h += typeof pl.media !== "undefined" ? '" media="' + pl.media + '"' : '';
		h += ' />';
		h += '</video>';
		p.innerHTML = "<!-- x --->" + h;
	}
...
function init() {
	...
	if ((/mceItem(Video|Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/).test(ed.dom.getAttrib(fe, 'class'))) {
		pl = fe.title;
	...
	document.getElementById('filebrowsercontainer').innerHTML = getBrowserHTML('filebrowser', 'src', 'media', 'mediahtml5');
	document.getElementById('qtsrcfilebrowsercontainer').innerHTML = getBrowserHTML('qtsrcfilebrowser', 'qt_qtsrc', 'media', 'mediahtml5');
	document.getElementById('bgcolor_pickcontainer').innerHTML = getColorPickerHTML('bgcolor_pick', 'bgcolor');
	var html = getMediaListHTML('medialist', 'src', 'media', 'mediahtml5');
	if (html === "") {
	...
	if (pl !== "") {
		...
		switch (type) {
		case "video":
			setStr(pl, 'video', 'mimetype');
			setStr(pl, 'video', 'poster');
			setStr(pl, 'video', 'title');
			setStr(pl, 'video', 'media');
			setBool(pl, 'video', 'showcontrols');
			setBool(pl, 'video', 'autoplay');
			setBool(pl, 'video', 'loop');
			setBool(pl, 'video', 'preload');
			setBool(pl, 'video', 'buffer');
			break;
		case "flash":
		...
		if (pl && (val = ed.dom.getAttrib(fe, "width")) !== "") {
			pl.width = f.width.value = val;
		}
		if ((val = ed.dom.getAttrib(fe, "height")) !== "") {
			pl.height = f.height.value = val;
		}
		oldWidth = pl.width ? parseInt(pl.width, 10) : 0;
		oldHeight = pl.height ? parseInt(pl.height, 10) : 0;
	} else {
...
function insertMedia() {
	...
	fe = ed.selection.getNode();
	if (fe !== null && (/mceItem(Video|Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/).test(ed.dom.getAttrib(fe, 'class'))) {
		// not sure when this gets called?
		switch (f.media_type.options[f.media_type.selectedIndex].value) {
		case "video":
			fe.className = "mceItemVideo";
			break;
		case "flash":
