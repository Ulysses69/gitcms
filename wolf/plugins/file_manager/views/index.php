...
<div id="files-path"><a href="<?php echo get_url('plugin/file_manager'); ?>">public</a> / <?php echo $out; ?></div>
...
<table id="files-list" class="index" cellpadding="0" cellspacing="0" border="0">
  <thead>
    <tr>
      ...
      <?php if(! AuthUser::hasPermission('editor')){ ?><th class="permissions"><?php echo __('Permissions'); ?></th>
      <th class="mtime"><?php echo __('Date'); ?></th><?php } ?>
      <th class="modify"><?php echo __('Action'); ?></th>
    </tr>
  </thead>
  <tbody>
<?php foreach ($files as $file): ?>
    <?php if($file->name != '/.svn' && $file->name != 'Thumbs.db' && $file->name != '.htaccess'): // prevent unwanted files/directories from being displayed ?>
	<tr class="<?php echo odd_even(); ?>"><td>
        <?php if ($file->is_dir) { ?>
      ...
      <?php if(! AuthUser::hasPermission('editor')){ ?><td><code><?php echo $file->perms; ?> (<a href="#" onclick="toggle_chmod_popup('<?php echo $dir.$file->name; ?>', '<?php echo $file->chmod; ?>'); return false;" title="<?php echo __('Change mode'); ?>"><?php echo $file->chmod; ?></a>)</code></td>
      <td><code><?php echo $file->mtime; ?></code></td><?php } ?>
      <td>
