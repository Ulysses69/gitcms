	...
	$j.extend($j.fn, {
		...
		miniColors: function(o, data) {
			...
			var create = function(input, o, data) {
				...
				// Create trigger
				var trigger = $j('<a class="miniColors-trigger" style="background-color: #' + color + '" href="#"></a>');
			...
			var show = function(input) {
				...
				var selector = $j('<div class="miniColors-selector"></div>');
				selector
				...
				$j('BODY').append(selector);
				selector.fadeIn(100);
				
				// Prevent text selection in IE
				selector.bind('selectstart', function() { return false; });
				
				$j(document).bind('mousedown.miniColors touchstart.miniColors', function(event) {
					
					input.data('mousebutton', 1);
					
					if( $j(event.target).parents().andSelf().hasClass('miniColors-colors') ) {
						event.preventDefault();
						input.data('moving', 'colors');
						moveColor(input, event);
					}
					
					if( $j(event.target).parents().andSelf().hasClass('miniColors-hues') ) {
						event.preventDefault();
						input.data('moving', 'hues');
						moveHue(input, event);
					}
					
					if( $j(event.target).parents().andSelf().hasClass('miniColors-selector') ) {
						event.preventDefault();
						return;
					}
					
					if( $j(event.target).parents().andSelf().hasClass('miniColors') ) return;
			...
			var hide = function(input) {
				...
				$j(input).each( function() {
					...
					$j(this).removeData('selector');
					$j(selector).fadeOut(100, function() {
						$j(this).remove();
					});
				});
			...
			var setColorFromInput = function(input) {
				...
				var colorPicker = $j(input.data('colorPicker'));
				colorPicker.css('top', colorPosition.y + 'px').css('left', colorPosition.x + 'px');
				input.data('colorPosition', colorPosition);
				
				// Set huePosition position
				var huePosition = getHuePositionFromHSB(hsb);
				var huePicker = $j(input.data('huePicker'));
			...
			// Handle calls to $j([selector]).miniColors()
			switch(o) {
			
				case 'readonly':
					
					$j(this).each( function() {
						if( !$j(this).hasClass('miniColors') ) return;
						$j(this).prop('readonly', data);
					});
				...
				default:
					...
					$j(this).each( function() {
						...
						if( $j(this)[0].tagName.toLowerCase() !== 'input' ) return;
						
						// If a trigger is present, the control was already created
						if( $j(this).data('trigger') ) return;
						
						// Create the control
						create($j(this), o, data);
