...

{
	 ...
	 */
	private
	function getStructure($source_path)
	{
		...
		if (isset($_POST['import']) || isset($_POST['preview'])) {
			...
			if (strlen(trim($_POST['options']['fileurl'])) > 0) {
				...
				if (!file_exists($source)) {
					Flash::set('error', __('File not found: :file:', array(
						':file:' => $filename
					)));
					redirect(get_url('plugin/csv_import'));
				}
			...
			foreach($structure['contents'] as $row) {
				...
				if (strlen($the_slug) > 0) {
					if (!in_array($the_slug, $used_slugs)) {
						$existingPage = Record::findOneFrom('Page', 'slug=? AND parent_id=?', array(
							$the_slug,
							$parent_page_id
						));
						...
						if (!$existingPage) {
							...
							$new_page->position = '0';
							if (!self::checkDateTime($new_page->created_on)) $new_page->created_on = $now_datetime;
							if (!self::checkDateTime($new_page->published_on)) $new_page->published_on = $now_datetime;
							...
							if ($new_page->save()) {
								$messages[] = array(
									'success',
									__('Row') . ' ' . $current_row . ': ' . 'PAGE IMPORTED - <b>' . $new_page->slug . '</b> <i>' . $new_page->title . '</i>'
								);
								...
								foreach($part_names as $head_key => $part_name) {
									...
									if ((strlen(trim($row[$head_key])) !== 0) || self::$options['create_empty_parts'] === '1') {
										...
										if ($new_page_part->save()) {
											$messages[] = array(
												'success',
												__('Row') . ' ' . $current_row . ': ' . __('Page') . ' <b>' . $new_page->slug . '</b>: ' . 'PART IMPORTED - <b>' . $new_page_part->name . '</b>'
											);
										} else $messages[] = array(
											'warning',
											__('Row') . ' ' . $current_row . ': ' . 'FAILED TO SAVE PART ' . $new_page_part->name . ' for page [' . $parent_page_id . ']'
										);

									} else $messages[] = array(
										'info',
										__('Row') . ' ' . $current_row . ': ' . 'PART EMPTY ' . $new_page_part->name . ' - NOT ADDED'
									);
								...
								'error',
								__('Row') . ' ' . $current_row . ': ' . 'Page not saved!'
							); // end new page save
							...
							'warning',
							__('Row') . ' ' . $current_row . ': ' . 'PAGE ALREADY EXISTS! slug = <b>' . $the_slug . '</b>'
						); // end checking if page exists

					} else $messages[] = array(
						'warning',
						__('Row') . ' ' . $current_row . ': ' . 'DUPLICATE SLUG - <b>' . $the_slug . '</b> - row ignored!'
					);

				} else $messages[] = array(
					'error',
					__('Row') . ' ' . $current_row . ': ' . 'EMPTY SLUG - row ignored!'
				);
	...
	function csvToArray($fileContent, $escape = '\\', $enclosure = '"', $delimiter = ';')
	{
		...
		else $fileContent = str_replace(array(
			"\r\n",
			"\r"
		) , array(
			"\n",
		...
		for ($i = 0; $i < $nb; $i++) {
			$c = $fileContent[$i];
			if ($c === $enclosure) {
				if ($previous !== $escape) $inEnclosure^= true;
				else $field.= $enclosure;
			}
			else
			if ($c === $escape) {
				$next = $fileContent[$i + 1];
				if ($next != $enclosure && $next != $escape) $field.= $escape;
			}
			else
			if ($c === $delimiter) {
				if ($inEnclosure) $field.= $delimiter;
				else {
	...
	private static
	function getAllChildren($page_id)
	{

		// Prepare SQL

		$sql = 'SELECT page.* ' . 'FROM ' . TABLE_PREFIX . 'page AS page ' . 'WHERE parent_id = ' . (int)$page_id . " ORDER BY page.position, page.id";
		$pages = array();
		Record::logQuery($sql);
	...
	private static
	function countAllChildren($page_id)
	{

		// Prepare SQL

		$sql = 'SELECT COUNT(*) AS nb_rows ' . 'FROM ' . TABLE_PREFIX . 'page AS page ' . 'WHERE parent_id = ' . (int)$page_id;
		Record::logQuery($sql);
	...
	function makePagesListRecursive($page_id = 1)
	{
	...
	function checkDateTime($sDate) {
		if ((preg_match('/^([0-9]{4})[-_]([0-9]{2})[-_]([0-9]{2}) ([0-1][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/D', (string)$sDate, $bits) && checkdate($bits[2], $bits[3], $bits[1])) === true) {
			return true;
		}
		else return false;
	}
	...
	function getFiles($dir) {
		...
		foreach($scandir as $k => $v) {
			if (strpos(strtolower(trim($v)) , '.csv') === false && strpos(strtolower(trim($v)) , '.tsv') === false && strpos(strtolower(trim($v)) , '.txt') === false) {
				unset($scandir[$k]);
			}
		}
